...
const EditVariant = ({ index, variants, setVariants }) => {
  const variantFormik = useFormik({
    initialValues: {
      color: variants[index]?.color || "",
      size: {
        length: variants[index]?.size?.length || "",
        width: variants[index]?.size?.width || "",
        height: variants[index]?.size?.height || "",
        stock: variants[index]?.size?.stock || "",
        price: variants[index]?.size?.price || "",
      },
    },
    ...
    onSubmit: (values) => {
      const newData = variants?.map((variant, key) =>
        key === index
          ? {
              color: values.color,
              size: {
